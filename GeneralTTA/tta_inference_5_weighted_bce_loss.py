...
# --------------------
# Weighted BCE Loss
def get_margin_weight(d_clean, d_blur, actual_margin=10.0, eps=1e-6):
    pct_diff = (d_blur - d_clean) / (d_blur + eps) * 100.0

    weight = torch.ones_like(pct_diff)
    weight[pct_diff < actual_margin] = 0.5

    return weight.detach()



class CustomMarginCleanBlurBCELoss(nn.Module):
    def __init__(self, sigma_min=5.0, sigma_max=20.0, weight=1.0):
        super().__init__()
        self.sigma_min = sigma_min
        self.sigma_max = sigma_max
    ...
    def forward(self, image_tensor, model):
        ...
        q_clean, _ = model(view_clean)
        q_blur, _  = model(view_blur) 
...
class CustomMarginCleanBlurBCELoss_Depre(nn.Module):
    def __init__(self, actual_margin=10.0):
        super().__init__()
        self.actual_margin = actual_margin
        self.bce = nn.BCELoss(reduction='none')

    def forward(self, d_clean, d_blur, q_clean, q_blur):
        """
        d_clean, d_blur: feature distances (B,)
        q_clean, q_blur: logits (B,)
        """
        p_clean = torch.sigmoid(q_clean)
        p_blur  = torch.sigmoid(q_blur)

        target_clean = torch.ones_like(p_clean)
        target_blur  = torch.zeros_like(p_blur)

        loss_clean = self.bce(p_clean, target_clean)
        loss_blur  = self.bce(p_blur, target_blur)

        # margin-aware weight
        weight = get_margin_weight(
            d_clean=d_clean,
            d_blur=d_blur,
            actual_margin=self.actual_margin
        )

        loss = weight * (loss_clean + loss_blur)
        return loss.mean()
...
class Model(object):
    ...
    def __init__(self, config, device, Net):
        ...
        # -----------------------
        # Weighted CleanBlurBCELoss
        # ------------------------
        self.weighted_bce_loss_fn = None
        if config.weighted_bce_loss:
            self.weighted_bce_loss_fn = CustomMarginCleanBlurBCELoss(
                sigma_min=getattr(config, 'clean_blur_sigma_min', 5.0),
                sigma_max=getattr(config, 'clean_blur_sigma_max', 20.0)
            )
            # Weight per image
    ...
    def adapt(self, data_dict, config, old_net):
        ...
        if config.weighted_bce_loss:
            ...
            sigmas_blur = []
            for _ in range(B):
                tmp_sigma_blur = sigma_min + np.random.rand() * (sigma_max - sigma_min)
                sigmas_blur.append(float(tmp_sigma_blur))
        ...
        for iteration in range(config.niter):
            ...
            if config.adaptive_margin_rank and (self.amrl is not None) and (q_high is not None) and (q_low is not None):
                ...
                # Compute AMRL loss
                amrl_loss = self.amrl(dist_high, dist_low, q_high_vec, q_low_vec)
                
                # Combine or replace loss
                if loss is not None and (config.rank or config.blur or config.comp or config.nos):
                    loss = loss + amrl_loss  # Combine with rank loss
                else:
                    loss = amrl_loss  # Use AMRL alone
                
                # loss_amrl_val = float(amrl_loss.detach().cpu().item())
                print(f"[loss] amrl_loss = {amrl_loss}")
                ...
                with open(args.logs_csv_path, mode="a", newline="") as f:
                    writer = csv.writer(f)
                    writer.writerow([timestamp, iteration, "adaptive_margin_rank", amrl_loss])
                    f.flush()
            ...
            if config.contrastive or config.contrique:
                ...
                contrastive_loss = loss_fn(f_neg_feat, f_pos_feat)
            ...
            # --------------------------------
            # Weighted Clean Blur BCE Loss
            # -----------------------------
            if config.weighted_bce_loss and self.weighted_bce_loss_fn is not None:
                f_neg_feat = self.ssh(f_low)
                f_pos_feat = self.ssh(f_high)
                # f_actual = self.ssh(inputs.cuda())

                dist_margin = torch.nn.PairwiseDistance(p=2)(f_pos_feat, f_neg_feat)

                def get_margin_weight(dist, tau_low=0.5, tau_high=1.5, eps=1e-6):
                    """
                    dist: Pairwise feature distance between clean & blur (B,)
                    """
                    # normalize
                    dist_norm = dist / (dist.mean() + eps)

                    w = (dist_norm - tau_low) / (tau_high - tau_low)
                    w = torch.clamp(w, 0.0, 1.0)

                    return w.detach()
                
                final_margin_dist = get_margin_weight(dist_margin)

                # compute statistics for logging
                eps = 1e-6
                dist_margin_cpu = dist_margin.detach().cpu()
                dist_margin_mean = float(dist_margin_cpu.mean().item())
                dist_margin_norm_list = (dist_margin_cpu / (dist_margin_mean + eps)).tolist()

                final_margin_dist_list = final_margin_dist.detach().cpu().tolist()

                # To set weight of CustomMarginCleanBlurBCELoss here for each image
                clean_blur_loss = self.weighted_bce_loss_fn(inputs.cuda(), self.net)
                self.weighted_bce_loss_fn.weight = final_margin_dist

                # image names from data_dict (could be list of paths)
                img_names = data_dict.get('img_name', None)
                if isinstance(img_names, (list, tuple)):
                    img_name_field = ';'.join([str(x) for x in img_names])
                else:
                    img_name_field = str(img_names)

                timestamp = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())

                # write to BCE CSV: image_name, iteration, dist_margin_mean, dist_margin_norm, final_margin_dist, loss_type, loss_value
                with open(args.bce_logs_csv_path, mode="a", newline="") as f:
                    writer = csv.writer(f)
                    writer.writerow([
                        timestamp,
                        img_name_field,
                        int(iteration),
                        float(dist_margin_mean),
                        ';'.join([str(x) for x in dist_margin_norm_list]),
                        ';'.join([str(x) for x in final_margin_dist_list])
                    ])
                    f.flush()
                ...
                # also log to general logs csv (keeps timestamp semantics)
                ...
                print("CustomCleanBlurBCELoss is skipped")
...
args.bce_logs_csv_path = os.path.join(args.exp_path,"bce_log.csv")
with open(args.bce_logs_csv_path, mode="w", newline="") as f:
    bce_writer = csv.writer(f)
    bce_writer.writerow(["timestamp", "image_name", "iteration", "dist_margin_mean", "dist_margin_norm", "final_margin_dist"])
