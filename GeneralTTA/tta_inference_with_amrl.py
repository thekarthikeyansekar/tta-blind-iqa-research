...
class Folder(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # NOTE: original code duplicated loader twice; keep processing once
        # To comment lines 129-134
        path, target = self.samples[index]
        sample = pil_loader(path)
        data_dict = {}
...
# -----------------------------
# Model and training/adapt logic
# -----------------------------
class Model(object):
    ...
    def __init__(self, config, device, Net):
        ...
        # Adaptive Margin Rank Loss flags and instance
        self.adaptive_margin_rank = getattr(config, 'adaptive_margin_rank', False)
        self.adaptive_gamma = getattr(config, 'adaptive_gamma', 0.5)
        if self.adaptive_margin_rank:
    ...
    def adapt(self, data_dict, config, old_net):
        ...
        with torch.no_grad():
            ...
            if config.rank:
                ...
                # compute predictions for all candidate augmentations (no grad)
                # with torch.no_grad():
                ...
                # id_score_dict for retrieving predicted scores corresponding to each augmentation
                id_score_dict = {0: pred1, 1: pred2, 2: pred3, 3: pred4, 4: pred5, 5: pred6}
                ...
                for p in range(len(pred0)):
                    sel = all_diff[p].argmax().item()
                    if sel == 0:
                        f_low.append(id_dict[1][p].cuda())
                        f_high.append(id_dict[0][p].cuda())
                        q_low_list.append(id_score_dict[1][p].cuda())
                        q_high_list.append(id_score_dict[0][p].cuda())
                    elif sel == 1:
                        f_low.append(id_dict[3][p].cuda())
                        f_high.append(id_dict[2][p].cuda())
                        q_low_list.append(id_score_dict[3][p].cuda())
                        q_high_list.append(id_score_dict[2][p].cuda())
                    elif sel == 2:
                        f_low.append(id_dict[5][p].cuda())
                        f_high.append(id_dict[4][p].cuda())
                        q_low_list.append(id_score_dict[5][p].cuda())
                        q_high_list.append(id_score_dict[4][p].cuda())
