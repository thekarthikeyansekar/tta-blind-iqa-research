...
![Block diagram of a general architecture for test time adaptation](https://github.com/subhadeeproy2000/assets/64764444/9213d944-23ba-4cdd-b49d-cc3e8e12678b)
# Installation 
```
...
TReS:
```
python TReS/weight/download_fblive.py
```
MUSIQ:
```
python MUSIQ/weights/download_fblive.py
python MUSIQ/model/download_fblive_resnet.py
```
HyperIQA:
```
python hyperIQA/weight/download_fblive.py
```
MetaIQA:
```
python MetaIQA/model_IQA/download.py
```
# Run Code for Four Different Models
TReS:
```
python TReS/ALL_EXPT.py
```
MUSIQ:
```
python MUSIQ/ALL_EXPT.py
```
HyperIQA:
```
python hyperIQA/ALL_EXPT.py
```
MetaIQA:
```
python MetaIQA/ALL_EXPT.py
```
# Generalize for any model
You need to write a code for developing the model. In the given demo, we write a code in TTA-IQA\GeneralTTA\models.py to design the architecture of TReS model.Then one need to specify where model should be classified into feature extractor and regressor ( In our case, two examples are shown. If we do not include transformer as a part of feature extractor then we will use SSHead.py, otherwise we will use SSHead_tf.py. Rest all the codes are same. Just need to look up whether you have a csv file along with names of the images in 'image_name' column and their corresponding MOS score in 'MOS' column. After providing all these codes, you can run tta_inference.py . For example -
```
python GeneralTTA/tta_inference.py --run 3 --batch_size 8 --lr 0.001 --niter 3 --svpath ttt_cifar_IQA/weight/fblive_TReS --gpunum 0 --test_patch_num 1 --fix_ssh --datapath DSLR --rank --group_contrastive
```

```
python GeneralTTA/tta_inference_with_amrl.py --rank --adaptive-margin-rank --adaptive-gamma 0.8
```
